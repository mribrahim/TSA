...
class Encoder(nn.Module):
    def __init__(self, configs, seq_len, pred_len):
        ...
        if configs.task_name == 'classification':
            self.linear_final = nn.Linear(self.feature_dim, configs.num_class)
        else:
...
class MSEFeedbackRNN(nn.Module):
    ...
    def forward(self, x):
        ...
        out, _ = self.rnn(x, h0)  # x should be (batch_size, sequence_length, input_size)
...
class Model(nn.Module):
    def __init__(self, configs):
        ...
        if self.task_name == 'anomaly_detection':
            ...
            self.rnn = MSEFeedbackRNN(input_size= self.feature_dim + 1, hidden_size=rnn_hidden_size, output_size=rnn_output_size, num_layers=8)            
        ...
        if self.task_name == 'classification':
            ...
            self.projection = nn.Linear(dim* configs.num_class, configs.num_class)           
        else: # short-term or long-term forecast
    ...
    def anomaly_detection(self, x_enc_input):
                
        x_enc = self.normalize_layer(x_enc_input, 'norm')
        ...
        mse_error = ((x_enc - output) ** 2).mean(dim=1, keepdim=True)
